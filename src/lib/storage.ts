...
export const migrateData = (data: any): FinanceData => {
  ...
  // Ensure favorites, and alertSettings exist
  ...
  if (!data.alertSettings) {
    ...
    if (data.alertSettings.thresholdOverrides === undefined)
      data.alertSettings.thresholdOverrides = {};
    if (data.alertSettings.dismissedItems === undefined)
      data.alertSettings.dismissedItems = [];
    if (data.alertSettings.dismissedTotal === undefined)
  ...
  const newBank = {
    ...defaultBank,
    initialBalance: data.initialBankBalance ?? data.initialBalance ?? 0,
  };
  const newCash = {
    ...defaultCash,
    initialBalance: data.initialCashBalance ?? 0,
  };
...
export const updateAlertSettings = (
  settings: Partial<FinanceData["alertSettings"]>,
): void => {
  const data = loadData();
  data.alertSettings = {
    ...(data.alertSettings || {
...
export const saveLocalSnapshot = (
  data: FinanceData,
  name: string = "Copia Automática",
) => {
  ...
  const totalBalance = accounts.reduce((sum, acc) => {
    ...
    const accTransactions = transactions.filter(
      (t) => t.accountId === acc.id && !t.isPending,
    );
    const balance = accTransactions.reduce(
      (accSum, t) =>
...
export const deleteAccount = (
  ...
  if (data.accounts.length <= 1) {
    return {
      success: false,
      message: "No puedes eliminar la única cuenta que queda.",
    };
  }
...
export const saveTransactionWithId = (transaction: Transaction): void => {
  const data = loadData();
  const existingIdx = data.transactions.findIndex(
...
export const addCategory = (categoryName: string): void => {
  ...
  const exists = (data.categories as any[]).some(
    (cat) =>
      normalizeCategory(typeof cat === "string" ? cat : cat.name) ===
      normalized,
  );
...
export const updateCategory = (updated: Category): void => {
  const data = loadData();
  const index = (data.categories as any[]).findIndex(
    (c) => (typeof c === "string" ? c : c.id) === updated.id,
  );
  if (index !== -1) {
    const oldCategory = data.categories[index];
    const oldName =
      typeof oldCategory === "string" ? oldCategory : oldCategory.name;
...
export const deleteCategory = (
  ...
  const data = loadData();
  const categoryToDelete = (data.categories as any[]).find(
    (c) => (typeof c === "string" ? c : c.id) === id,
  );
  const categoryName =
    typeof categoryToDelete === "string"
      ? categoryToDelete
  ...
  if (hasTransactions) {
    return {
      success: false,
      message: "No se puede eliminar una categoría con transacciones.",
    };
  }

  data.categories = (data.categories as any[]).filter(
    (c) => (typeof c === "string" ? c : c.id) !== id,
  );
  saveData(data);
  return { success: true };
...
export const findSimilarCategory = (
  input: string,
...
export const getCategorySuggestions = (
  ...
  return categories
    ...
    .filter((name) => normalizeCategory(name).includes(normalized));
};
